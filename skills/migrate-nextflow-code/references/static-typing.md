...
4. **SWAP LEGACY OPERATORS** — Replace `set`/`tap`, `.out`, `|`/`&`, `branch`, `multiMap`, `groupTuple`, operator-form `splitCsv`, and capitalized `Channel.` factories per the [operators section](#operators-under-typing).
5. **MIGRATE PARAMS** — Move script-used params into a typed `params {}` block in the entry-workflow file and propagate them as explicit inputs; do not read the global `params` object inside subworkflows/processes.
